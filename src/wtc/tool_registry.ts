...
const globDescription =
  'Fast file pattern matching tool that works with any codebase size\n- Prefer passing `path` as the base directory and `pattern` as a relative glob, for example `path: "/${LorebookName}"` with `pattern: "*"` or `path: "/Schemas"` with `pattern: "*.json"`\n- If you omit `path`, this tool also accepts absolute-style patterns like "/${LorebookName}/[mvu_update]*", "/${LorebookName}/*", or "/Schemas/*" and will automatically split the parent path into `path`\n- Returns matching file paths sorted by modification time\n- Use this tool when you need to find files by name patterns\n- When you are doing an open ended search that may require multiple rounds of globbing and grepping, use the Agent tool instead\n- You can call multiple tools in a single response. It is always better to speculatively perform multiple searches in parallel if they are potentially useful.';
